...
fn stub_runtime_for_clippy() -> Result<()> {
...
fn copy_runtime() -> Result<()> {
    ...
    let engine_path = module_path.join("mjsc_core.wasm");
    let quickjs_provider_path = module_path.join("mjsc_quickjs_provider.wasm");
    let quickjs_provider_wizened_path = module_path.join("mjsc_quickjs_provider_wizened.wasm");
